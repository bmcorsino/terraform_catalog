...
[Parameter(Mandatory=$true,HelpMessage="Enter the value for Schedule. Values can be either stop or start")][String]$Schedule,
[Parameter(Mandatory=$true,HelpMessage="Enter the value for Timezone. Values can be either stop or start")][String]$Timezone
)
#---------code block---------------------------------
...
function getListRmResource([string]$TagName = "schedule", [string]$TagValue, [string]$ResourceGroup){
    $ResultVMList=@()
    $taggedVMs = Get-AzureRmResource -TagName "$($TagName)" -TagValue "$($TagValue)" -ResourceType "Microsoft.Compute/virtualMachines"

    foreach($vmResource in $taggedVMs) {
        $status = Get-AzureRmVM -ResourceGroupName "$($vmResource.ResourceGroupName)" -Name "$($vmResource.Name)" -Status | select -ExpandProperty Statuses | ?{ $_.Code -match "PowerState" } | select -ExpandProperty code
        $ResultVMList += @{Name = $vmResource.Name; ResourceGroupName = $vmResource.ResourceGroupName; Type = "ResourceManager"; "$($TagName)" = $vmResource.Tags[$TagName]; "Timezone" = $vmResource.Tags['timezone']; "Status" = $status}
    }
    return $ResultVMList
}
...
try {
    ...
    $Schedule = $Schedule.Trim().ToLower()
    $Timezone = $Timezone.Trim().ToUpper()
    if (!($Action -eq "start" -or $Action -eq "stop")) {throw "Action '$Action' not valid!"}
    ...
    $AzureVMListARM=@()
    $AzureVMList=@()
    $AzureVMList += getListRmResource -TagName "$TAG" -TagValue "$Schedule"

    foreach($vmResource in $AzureVMList) {
        if ($vmResource.ResourceGroupName -eq $null) { continue }
        if ($vmResource.Timezone.Trim().ToUpper() -ne $Timezone) { continue }

        if (($Action -eq "start") -and ($vmResource.Status.Trim().ToUpper() -like "*running*")) { continue }
        if (($Action -eq "stop") -and ($vmResource.Status.Trim().ToUpper() -like "*deallocated*")) { continue }

        # Add instance valid
        $AzureVMListARM += $vmResource
    }

    Write-Output "======================================================================"
    Write-Output "Found $($AzureVMListARM.Count) VMs to run $($Action.ToUpper())"
    Write-Output "======================================================================"

    if ($Action -eq 'start') {
        $AzureVMListARM | Invoke-Parallel -ImportVariables -NoCloseOnTimeout -ScriptBlock {
            Write-Output "Starting $($_.Name) in Resource Group $($_.ResourceGroupName)"
            Start-AzureRMVM -ResourceGroupName "$($_.ResourceGroupName)" -Name "$($_.Name)" -Verbose
        }
    }

    if ($Action -eq 'stop') {
        $AzureVMListARM | Invoke-Parallel -ImportVariables -NoCloseOnTimeout -ScriptBlock {
            Write-Output "Stopping $($_.Name) in Resource Group $($_.ResourceGroupName)"
